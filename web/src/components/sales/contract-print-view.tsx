...
import type { QuotationPrintOrg, QuotationPrintLine } from "@/components/sales/quotation-print-view";

export type ContractPrintViewProps = {
...
export function ContractPrintView({
  ...
  return (
    <div className="sales-contract-print-root mx-auto max-w-[210mm] bg-white p-10 text-zinc-900 shadow-sm print:shadow-none print:p-8">
      <header className="border-b border-zinc-300 pb-4">
        <div className="flex justify-between gap-6">
          <div>
            <h1 className="text-2xl font-bold tracking-tight">銷售合同</h1>
            <p className="mt-1 text-xs text-zinc-500">SALES CONTRACT</p>
          </div>
        ...
        <div className="text-right">
          <dl className="space-y-1">
            <div className="flex justify-end gap-3">
              <dt className="text-zinc-500">合同編號</dt>
              <dd className="font-mono font-medium">{contractNo}</dd>
            </div>
            <div className="flex justify-end gap-3">
            ...
            <div className="flex justify-end gap-3">
              <dt className="text-zinc-500">合同日</dt>
              <dd>{contractDate}</dd>
            </div>
            <div className="flex justify-end gap-3">
            ...
            {sourceQuoteNo ? (
              <div className="flex justify-end gap-3">
                <dt className="text-zinc-500">來源報價單</dt>
                <dd className="font-mono text-xs">{sourceQuoteNo}</dd>
              </div>
            ) : null}
            {proformaInvoiceNo ? (
              <div className="flex justify-end gap-3">
                <dt className="text-zinc-500">預收發票</dt>
                <dd className="font-mono text-xs">{proformaInvoiceNo}</dd>
              </div>
            ) : null}
          </dl>
        </div>
      ...
      <div className="mt-6 flex justify-end">
        <dl className="w-72 space-y-1 text-sm">
          ...
          {prepaymentAmount != null && Number.isFinite(prepaymentAmount) && prepaymentAmount > 0 ? (
            <div className="flex justify-between border-t border-dashed border-zinc-200 pt-2 text-zinc-700">
              <dt>約定預收款（含稅）</dt>
              <dd className="tabular-nums font-medium">{fmtMoney(prepaymentAmount)}</dd>
            </div>
          ) : null}
        </dl>
      </div>

      {prepaymentNotes?.trim() ? (
        <section className="mt-6 border-t border-zinc-200 pt-4 text-sm">
          <h2 className="text-xs font-semibold uppercase tracking-wide text-zinc-500">預收款備註</h2>
          <p className="mt-2 whitespace-pre-wrap text-zinc-700">{prepaymentNotes}</p>
        </section>
      ) : null}
