...
export type ProformaInvoicePrintViewProps = {
...
export function ProformaInvoicePrintView({
  org,
  invoiceNo,
  issueDate,
  sourceContractNo,
  ...
  totalAmount,
  notes,
}: ProformaInvoicePrintViewProps) {
  return (
    <div className="proforma-invoice-print-root mx-auto max-w-[210mm] bg-white p-10 text-zinc-900 shadow-sm print:shadow-none print:p-8">
      <header className="border-b border-zinc-300 pb-4">
        <div className="flex justify-between gap-6">
          <div>
            <h1 className="text-2xl font-bold tracking-tight">預收發票</h1>
            <p className="mt-1 text-xs text-zinc-500">PROFORMA INVOICE</p>
          </div>
        ...
        <div className="text-right">
          <dl className="space-y-1">
            <div className="flex justify-end gap-3">
              <dt className="text-zinc-500">發票編號</dt>
            ...
            <div className="flex justify-end gap-3">
              <dt className="text-zinc-500">開立日</dt>
              <dd>{issueDate}</dd>
            </div>
            <div className="flex justify-end gap-3">
      ...
      <footer className="mt-16 border-t border-zinc-200 pt-6 text-center text-xs text-zinc-500">
        <p>本單據為預收性質之發票／請款說明，實際收款以財務入帳為準。</p>
      </footer>
    </div>
  );
}
