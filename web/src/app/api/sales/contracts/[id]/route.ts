import { eq } from "drizzle-orm";
import { NextRequest, NextResponse } from "next/server";
import { z } from "zod";

import { auth } from "@/auth";
import { getDb, getNeonSql } from "@/db";
import { customers, salesContracts, type QuotationLineItem } from "@/db/schema";
import { canEditSales } from "@/lib/authz";
import { loadSalesContractDetail } from "@/lib/sales/load-sales-contract";
import { normalizeItemsForPersist, totalsFromLines } from "@/lib/sales/quotation-math";
import { normalizeNeonRows } from "@/lib/sales/quotations-list-fallback";

export const runtime = "nodejs";

const prepayOnlySchema = z.object({
  prepayment_amount: z.coerce.number().min(0).optional(),
  prepayment_notes: z.string().optional().nullable(),
});

const lineSchema = z.object({
  product_id: z.string().uuid().optional().nullable(),
  sku: z.string().optional().nullable(),
  name: z.string().min(1),
  qty: z.coerce.number(),
  unit_price: z.coerce.number(),
  discount: z.coerce.number().min(0).max(100).optional().default(0),
});

const fullContractPatchSchema = z.object({
  customer_id: z.string().uuid(),
  contract_date: z.string().regex(/^\d{4}-\d{2}-\d{2}$/),
  valid_until: z.string().regex(/^\d{4}-\d{2}-\d{2}$/),
  tax_rate: z.coerce.number().min(0).max(100),
  items: z.array(lineSchema).min(1),
  notes: z.string().optional().nullable(),
  status: z.enum(["Active", "Completed", "Cancelled"]),
  prepayment_amount: z.coerce.number().min(0).optional(),
  prepayment_notes: z.string().optional().nullable(),
});

function isFullContractPatchBody(body: unknown): body is Record<string, unknown> {
  return (
    body !== null &&
    typeof body === "object" &&
    "items" in body &&
    Array.isArray((body as Record<string, unknown>).items)
  );
}

export async function GET(_req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  const session = await auth();
  if (!session?.user?.id) {
    return NextResponse.json({ error: "未授權" }, { status: 401 });
  }

  const { id: rawId } = await ctx.params;
  const idParsed = z.string().uuid().safeParse(rawId);
  if (!idParsed.success) {
    return NextResponse.json({ error: "無效的合同 id" }, { status: 400 });
  }

  const item = await loadSalesContractDetail(idParsed.data);
  if (!item) {
    return NextResponse.json({ error: "找不到銷售合同" }, { status: 404 });
  }

  return NextResponse.json({ item });
}

export async function PATCH(req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  const session = await auth();
  if (!session?.user?.id) {
    return NextResponse.json({ error: "未授權" }, { status: 401 });
  }
  if (!canEditSales(session)) {
    return NextResponse.json({ error: "無權限修改銷售合同" }, { status: 403 });
  }

  const { id: rawId } = await ctx.params;
  const idParsed = z.string().uuid().safeParse(rawId);
  if (!idParsed.success) {
    return NextResponse.json({ error: "無效的合同 id" }, { status: 400 });
  }

  let body: unknown;
  try {
    body = await req.json();
  } catch {
    return NextResponse.json({ error: "請求內容須為 JSON" }, { status: 400 });
  }

  if (isFullContractPatchBody(body)) {
    const parsed = fullContractPatchSchema.safeParse(body);
    if (!parsed.success) {
      return NextResponse.json({ error: "資料驗證失敗", issues: parsed.error.flatten() }, { status: 400 });
    }
    const data = parsed.data;

    try {
      const db = getDb();
      const [existing] = await db
        .select({ id: salesContracts.id })
        .from(salesContracts)
        .where(eq(salesContracts.id, idParsed.data))
        .limit(1);
      if (!existing) {
        return NextResponse.json({ error: "找不到銷售合同" }, { status: 404 });
      }

      const [cust] = await db
        .select({
          id: customers.id,
          name: customers.name,
          phone: customers.phone,
          email: customers.email,
        })
        .from(customers)
        .where(eq(customers.id, data.customer_id))
        .limit(1);

      if (!cust) {
        return NextResponse.json({ error: "指定的客戶不存在" }, { status: 400 });
      }

      const rawItems: QuotationLineItem[] = data.items.map((l) => ({
        product_id: l.product_id,
        sku: l.sku,
        name: l.name.trim(),
        qty: l.qty,
        unit_price: l.unit_price,
        discount: l.discount ?? 0,
      }));

      const itemsNorm = normalizeItemsForPersist(rawItems);
      if (itemsNorm.length === 0) {
        return NextResponse.json({ error: "至少需一筆有效品項" }, { status: 400 });
      }

      const lineTotals = itemsNorm.map((l) => l.line_total ?? 0);
      const { subtotal, tax_amount, total_amount } = totalsFromLines(lineTotals, data.tax_rate);

      await db
        .update(salesContracts)
        .set({
          customerId: data.customer_id,
          customerName: cust.name,
          customerPhone: cust.phone,
          customerEmail: cust.email,
          contractDate: data.contract_date,
          validUntil: data.valid_until,
          items: itemsNorm,
          subtotal: String(subtotal),
          taxRate: String(data.tax_rate),
          taxAmount: String(tax_amount),
          totalAmount: String(total_amount),
          notes: data.notes?.trim() ? data.notes.trim() : null,
          status: data.status,
          ...(data.prepayment_amount !== undefined
            ? { prepaymentAmount: String(data.prepayment_amount) }
            : {}),
          ...(data.prepayment_notes !== undefined
            ? {
                prepaymentNotes: data.prepayment_notes?.trim() ? data.prepayment_notes.trim() : null,
              }
            : {}),
        })
        .where(eq(salesContracts.id, idParsed.data));

      const item = await loadSalesContractDetail(idParsed.data);
      return NextResponse.json({ item });
    } catch (e) {
      const msg = e instanceof Error ? e.message : String(e);
      console.error("[PATCH /api/sales/contracts/[id]] full", msg);
      return NextResponse.json(
        {
          error: "無法更新銷售合同",
          detail: process.env.NODE_ENV !== "production" ? msg : undefined,
          hint: /prepayment|does not exist/i.test(msg) ? "請執行 npm run db:apply:contracts-prepayment" : undefined,
        },
        { status: 500 }
      );
    }
  }

  const parsedPrepay = prepayOnlySchema.safeParse(body);
  if (!parsedPrepay.success) {
    return NextResponse.json({ error: "資料驗證失敗", issues: parsedPrepay.error.flatten() }, { status: 400 });
  }

  const data = parsedPrepay.data;
  if (data.prepayment_amount === undefined && data.prepayment_notes === undefined) {
    return NextResponse.json({ error: "無可更新欄位" }, { status: 400 });
  }

  try {
    const db = getDb();
    const [existing] = await db
      .select({ id: salesContracts.id })
      .from(salesContracts)
      .where(eq(salesContracts.id, idParsed.data))
      .limit(1);
    if (!existing) {
      return NextResponse.json({ error: "找不到銷售合同" }, { status: 404 });
    }

    const setObj: Partial<{
      prepaymentAmount: string | null;
      prepaymentNotes: string | null;
    }> = {};
    if (data.prepayment_amount !== undefined) {
      setObj.prepaymentAmount = String(data.prepayment_amount);
    }
    if (data.prepayment_notes !== undefined) {
      setObj.prepaymentNotes = data.prepayment_notes?.trim() ? data.prepayment_notes.trim() : null;
    }

    await db.update(salesContracts).set(setObj).where(eq(salesContracts.id, idParsed.data));

    const [row] = await db
      .select({
        id: salesContracts.id,
        contractNo: salesContracts.contractNo,
        prepaymentAmount: salesContracts.prepaymentAmount,
        prepaymentNotes: salesContracts.prepaymentNotes,
      })
      .from(salesContracts)
      .where(eq(salesContracts.id, idParsed.data))
      .limit(1);

    return NextResponse.json({ item: row });
  } catch (e) {
    const msg = e instanceof Error ? e.message : String(e);
    console.error("[PATCH /api/sales/contracts/[id]] prepay", msg);
    return NextResponse.json(
      {
        error: "無法更新銷售合同",
        detail: process.env.NODE_ENV !== "production" ? msg : undefined,
        hint: /prepayment|does not exist/i.test(msg) ? "請執行 npm run db:apply:contracts-prepayment" : undefined,
      },
      { status: 500 }
    );
  }
}

export async function DELETE(_req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  const session = await auth();
  if (!session?.user?.id) {
    return NextResponse.json({ error: "未授權" }, { status: 401 });
  }
  if (!canEditSales(session)) {
    return NextResponse.json({ error: "無權限刪除銷售合同" }, { status: 403 });
  }

  const { id: rawId } = await ctx.params;
  const idParsed = z.string().uuid().safeParse(rawId);
  if (!idParsed.success) {
    return NextResponse.json({ error: "無效的合同 id" }, { status: 400 });
  }

  try {
    const db = getDb();
    const [existing] = await db
      .select({ id: salesContracts.id })
      .from(salesContracts)
      .where(eq(salesContracts.id, idParsed.data))
      .limit(1);
    if (!existing) {
      return NextResponse.json({ error: "找不到銷售合同" }, { status: 404 });
    }

    await db.delete(salesContracts).where(eq(salesContracts.id, idParsed.data));
    return NextResponse.json({ ok: true });
  } catch (e) {
    const msg = e instanceof Error ? e.message : String(e);
    try {
      const sql = getNeonSql();
      const raw = await sql.query(`DELETE FROM sales_contracts WHERE id = $1::uuid RETURNING id`, [idParsed.data]);
      const deleted = normalizeNeonRows<{ id: string }>(raw);
      if (deleted.length === 0) {
        return NextResponse.json({ error: "找不到銷售合同" }, { status: 404 });
      }
      console.warn("[DELETE /api/sales/contracts/[id]] Drizzle 失敗，已改用 SQL:", msg);
      return NextResponse.json({ ok: true });
    } catch (e2) {
      const msg2 = e2 instanceof Error ? e2.message : String(e2);
      console.error("[DELETE /api/sales/contracts/[id]]", msg, msg2);
      return NextResponse.json(
        { error: "無法刪除銷售合同", detail: process.env.NODE_ENV !== "production" ? `${msg} | ${msg2}` : undefined },
        { status: 500 }
      );
    }
  }
}
