...
export async function GET(_req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  if (!idParsed.success) {
    return NextResponse.json({ error: "inventory id 須為有效 UUID" }, { status: 400 });
  }

  try {
    const sql = getNeonSql();
    const rows = await sql.query(
      `SELECT r.id, r.purchase_order_id, po.po_no, r.sku, r.qty_received,
       ...
       INNER JOIN inventory i ON i.id = $1::uuid
         AND r.product_id = i.product_id
         AND (r.warehouse_location IS NOT DISTINCT FROM i.warehouse_location)
    ...
    console.error("[GET /api/inventory/[id]/linked-receipts]", msg);
    return NextResponse.json(
      {
        error: "無法讀取關聯採購入庫",
        detail: process.env.NODE_ENV !== "production" ? msg : undefined,
        hint: /inventory|purchase_order_receipts|does not exist/i.test(msg)
