/**
 * 套用 sql/sales_contracts_init.sql
...
async function main() {
  ...
  const sql = neon(url);
  const path = resolve(process.cwd(), "sql", "sales_contracts_init.sql");
  ...
  console.log(`Applied sales_contracts_init.sql (${statements.length} statements).`);
}
