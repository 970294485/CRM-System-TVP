/**
 * 套用 sql/finance_payment_advance_init.sql
...
async function main() {
  ...
  const sql = neon(url);
  const path = resolve(process.cwd(), "sql", "finance_payment_advance_init.sql");
  ...
  console.log(`Applied finance_payment_advance_init.sql (${statements.length} statements).`);
}
