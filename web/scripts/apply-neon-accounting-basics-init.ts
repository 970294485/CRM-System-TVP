/**
 * 套用 sql/accounting_basics_init.sql（會計基礎管理表）
 * web/: npm run db:apply:accounting-basics-init
 */
...
async function main() {
  ...
  const sql = neon(url);
  const path = resolve(process.cwd(), "sql", "accounting_basics_init.sql");
  ...
  console.log(`Applied accounting_basics_init.sql (${statements.length} statements).`);
}
