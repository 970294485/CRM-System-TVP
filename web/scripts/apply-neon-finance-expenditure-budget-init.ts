/**
 * 套用 sql/finance_monthly_expenditure_budget_init.sql
 * web/: npm run db:apply:finance-expenditure-budget
 */
...
async function main() {
  ...
  const sql = neon(url);
  const path = resolve(process.cwd(), "sql", "finance_monthly_expenditure_budget_init.sql");
  ...
  console.log(`Applied finance_monthly_expenditure_budget_init.sql (${statements.length} statements).`);
}
