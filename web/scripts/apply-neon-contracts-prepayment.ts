/**
 * 套用 sql/sales_contracts_prepayment_proforma.sql
...
async function main() {
  ...
  const sql = neon(url);
  const path = resolve(process.cwd(), "sql", "sales_contracts_prepayment_proforma.sql");
  ...
  console.log(`Applied sales_contracts_prepayment_proforma.sql (${statements.length} statements).`);
}
