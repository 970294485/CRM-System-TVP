/**
 * 僅套用 sql/quotations_sales_management_alter.sql
 * web/: npm run db:apply:quotations-sales
 */
...
const DATABASE_URL: string = raw;

async function main() {
  const url = sanitizeDatabaseUrl(DATABASE_URL.charCodeAt(0) === 0xfeff ? DATABASE_URL.slice(1).trim() : DATABASE_URL);
  const sql = neon(url);
  const path = resolve(process.cwd(), "sql", "quotations_sales_management_alter.sql");
  ...
  console.log(`Applied quotations_sales_management_alter.sql (${statements.length} statements).`);
}
